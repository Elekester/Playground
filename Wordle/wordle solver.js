...
function update_wordle(guess, values) {
...
function guess_wordle(wordle, word_list) {
	...
	wordle_guesses = wordle_guesses.filter(filter_correct).filter(filter_wrong_spot).filter(filter_incorrect);

	return wordle_guesses;
}

function do_wordle(guess, values) {
	update_wordle(guess, values);
	return guess_wordle(wordle, word_list)[0];
}
